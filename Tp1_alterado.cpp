...
void jogar_humano(){
	...
	while (ganhou!=true && jogada<9){
		...
		if (fim==false){
			if (turno==1){
				while (certo == false){
					cout << "Escolha a linha e coluna que deseja(jogador1): ";
					cin >> tecla;
					teclado(tecla);
				...
				certo=false;
				while (certo==false){
					cout << "Escolha a linha e coluna que deseja(jogador2): ";
					cin >> tecla;
					teclado(tecla);
					if (M[m_linha][m_coluna]==' '){
...
void replay(){
	...
	while(novo != 'n'){	
		...
		else if (novo=='n'||novo=='N'){ 
			cout << endl;
			cout << "Obrigado por jogar :-D Veja agora o resultado final do placar. \n\n";
		}
		
		else if ((novo!='s'||novo!='S') && (novo!='n'||novo!='N')){
			cout << "Que? Nao entendi a tecla. "; 
		}
		replay();
	}	
}
