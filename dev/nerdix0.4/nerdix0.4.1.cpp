...
struct game{// quase todas variaveis que as diversas funções controlam por referência estão aqui.
	char M[3][3];
	int linha, coluna, turno, roda, velha;
	bool vitoria;
};
	//funções que não recebem nada:
	void intro();// animação de abertura.
	void apagar();// imprime um monte de couts de espaços vazios para apagar a tela.
	//funções que recebem (manipulam por referência) apenas player:
	...
	//funções que recebem player e game (tudo):
	void jogo_da_velha(player*, player*, game*); //contém a ordem das funções para acontecer o jogo.
	void jogar_humano(player*, player*, game*); //jogada do player humano.
	void jogar_robo(player*, player*, game*); //jogada do player robô.
...
int main(){ // MAIN MAIN MAIN MAIN MAIN MAINMAINMAINMAINMAINMAINMAINMAINMAINMAINMAIN
	...
	//atribuição dos valores iniciais às variáveis.
	X.score=0;
	O.score=0;
	nerd.velha=0;
	nerd.roda=1;
...
void jogar_humano(player *X, player *O, game *nerd){
	bool certo=false;
...
void jogar_robo(player *X, player *O, game *nerd){
	srand(time(NULL));
	char tecla;
	bool certo=false;
